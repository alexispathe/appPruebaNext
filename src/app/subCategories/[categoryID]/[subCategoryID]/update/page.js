...
const UpdateSubcategory = ({ params }) => {
  ...
  const categoryID = "odzhVY9kAqYOpfCzMv5F";
  const subCategoryID = "Zo1A0i5Am56B9VUFBLEo"
  ...
  const loadSubcategoryData = async (categoryID, subCategoryID) => {
    try {
      const token = await auth.currentUser.getIdToken();
      const response = await fetch(`/api/subCategories/${categoryID}/${subCategoryID}/get`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const token = await auth.currentUser.getIdToken();
    const response = await fetch(`/api/subCategories/${categoryID}/${subCategoryID}/update`, {
      method: 'PUT',
      headers: {
